...
function isPalindrome(str) {
  const regularExp = /[^A-Za-z0-9]/g;
  const lowerStr = str.toLowerCase().replace(regularExp, '');
...
function findLongestWord(sentence) {
  ...
  const longestWord = arrayWords.sort((a, b) => b.length - a.length);
...
function encodeToRot13(str) {
  const alphabetBefore = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const alphabetAfter = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const displacement = (letter) =>
    alphabetBefore.indexOf(letter) !== -1
      ? alphabetAfter[alphabetBefore.indexOf(letter)]
      : letter;
  return str.split('').map(displacement).join('');
}
